//----------------------------------------------------------------------------
// Example
//----------------------------------------------------------------------------

module add
(
  input  [3:0] a, b,
  output [3:0] sum
);

  assign sum = a + b;

endmodule

//----------------------------------------------------------------------------
// Task
//----------------------------------------------------------------------------

module signed_add_with_saturation
(
  input  [3:0] a, b,
  output [3:0] sum
);

  // Task:
  //
  // Implement a module that adds two signed numbers with saturation.
  //
  // "Adding with saturation" means:
  //
  // When the result does not fit into 4 bits,
  // and the arguments are positive,
  // the sum should be set to the maximum positive number.
  //
  // When the result does not fit into 4 bits,
  // and the arguments are negative,
  // the sum should be set to the minimum negative number.
  wire signed [3:0] a_signed = a;
  wire signed [3:0] b_signed = b;
  wire signed [4:0] extended_sum;
  wire overflow_positive, overflow_negative;
  assign extended_sum = {a_signed[3], a_signed} + {b_signed[3], b_signed};
  assign overflow_positive = (~a_signed[3] & ~b_signed[3] & extended_sum[3]);
  assign overflow_negative = (a_signed[3] & b_signed[3] & ~extended_sum[3]);
  assign sum = overflow_positive ? 4'sd7 :  
               overflow_negative ? 4'sd8 :  
               extended_sum[3:0]; 

endmodule
